 ...
 * @constructor
 */
var PromoterController = function($scope, $http) {
    $scope.rs = {};
    $scope.editMode = false;

    $scope.fetchRailwayStationsList = function() {
        $http.get('promoter/promoterlist.json').success(function(rsList){
    ...
    $scope.addNewRailwayStation = function(rs) {
        ...
        $http.post('promoter/add', rs).success(function() {
    ...
    $scope.updateRailwayStation = function(rs) {
        ...
        $http.put('promoter/update', rs).success(function() {
            $scope.fetchRailwayStationsList();
            $scope.rs.name = '';
            $scope.rs.train.name = '';
            $scope.rs.train.speed = '';
    ...
    $scope.removeRailwayStation = function(id) {
        ...
        $http.delete('promoter/remove/' + id).success(function() {
            $scope.fetchRailwayStationsList();
        }).error(function() {
    ...
    $scope.removeAllRailwayStations = function() {
        ...
        $http.delete('promoter/removeAll').success(function() {
            $scope.fetchRailwayStationsList();
        }).error(function() {
